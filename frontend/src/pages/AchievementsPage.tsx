  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {[
          { title: 'First Steps', desc: 'Log your first workout', icon: <Star />, bg: 'from-[var(--color-text-base)] to-[var(--color-text-muted)]', shadow: 'shadow-md', unlocked: true },
          { title: 'Habit Hero', desc: 'Complete all habits for 7 days', icon: <Trophy />, bg: 'from-[var(--color-text-base)] to-[var(--color-text-muted)]', shadow: 'shadow-md', unlocked: false },
          { title: 'Iron Will', desc: 'Workout 5 times in a week', icon: <Shield />, bg: 'from-[var(--color-text-base)] to-red-600', shadow: 'shadow-md', unlocked: false },
          { title: 'Consistent', desc: 'Log weight 30 days', icon: <Award />, bg: 'from-[var(--color-text-base)] to-teal-600', shadow: 'shadow-md', unlocked: false },
        ].map((a, i) => (
          <div key={i} className={`glass-card p-5 flex flex-col items-center text-center transition-all duration-300 ${a.unlocked ? 'glass-card-hover scale-100 hover:scale-105' : 'opacity-40 grayscale scale-95'}`}>
            ...
            {a.unlocked && <span className="mt-3 text-[10px] font-black tracking-wider uppercase text-[var(--color-text-muted)] bg-[var(--color-surface)] px-2 py-0.5 rounded border border-[var(--color-border-subtle)]">Unlocked</span>}
